<template>
  <div :class="{'h-full': registred}">
    <template v-if="!registred">
      ...
      <p class="mt-6">Bienvenu dans un batiment rempli d'invités.</p>
      <p class="mt-2">Avant de commencer ton aventure, qui es-tu ?</p>

      <div
          class="mt-6 flex items-center justify-between h-full border-4 rounded pb-2 px-2 border-double border-borders">
        <p class="font-bold self-center mt-2">{{ currentGuest.name }}</p>
    ...
    <div v-else class="space-y-8 flex flex-col justify-center h-full">
      <h1 class="text-lg text-center">Bienvenue {{ currentGuestFirstName }} !</h1>
...
$mitt.on('A_PRESS', () => {
  ...
  setTimeout(() => emit('next', MenuScreen), 3200)
})
